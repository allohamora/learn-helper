...
describe('getContext', () => {
  ...
  it('behaves the same with real semantic tags (h1 + p) as with generic PDF-style spans', () => {
    // the algorithm never inspects tag names, only structure and text - this proves that isn't just
    // true in theory by swapping the generic <span> siblings above for the tags an ordinary blog or
    // article page would actually use.
    const article = document.createElement('div');
    const heading = document.createElement('h1');
    heading.textContent = 'Field Notes';
    const paragraph = document.createElement('p');
    paragraph.textContent = 'This report explains what happened during the outage and how it was resolved.';
    ...
    const end = start + 'report'.length;
    const selection = selectRange((range) => {
      range.setStart(textNode, start);
      range.setEnd(textNode, end);
    });

    expect(getContext(selection)).toBe('This report explains what happened during the outage and how it was resolved.');
