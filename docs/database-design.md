...
> **Deployment note:** The `uploads/` directory must be backed by a PersistentVolumeClaim (see `k8s/learn-helper.yaml`) so uploaded files survive Pod restarts and rescheduling.
