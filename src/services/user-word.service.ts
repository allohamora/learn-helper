...
export const toFillInTheGap = async (words: WordData[]) => {
  const { reasoning, output, usage } = await generateText({
    ...
    prompt: [
      ...
      '- Sentence: 3-15 words, natural and grammatically correct, with exactly one "___" blank; target word appears only in the blank',
      '- Use appropriate punctuation (., !, ?) for sentence type',
