...
export const toTranslateEnglishSentence = async (words: WordData[]) => {
  const { reasoning, output, usage } = await generateText({
    ...
    prompt: [
      ...
      '<requirements>',
      '- Id: task.id matches input word.id.',
      '- English sentence: max 15 words, natural, capitalized, includes the exact target phrase. If a minimal variant is needed, limit to inflection/conjugation of verbs or auxiliaries within the target (e.g., "be going to" -> "is going to"); do not swap articles or other function words.',
      '- Ukrainian translation: max 15 words, natural and grammatical, capitalized, single spaces, punctuation attached to tokens; internal commas allowed; final punctuation attached to the last word.',
      '- Use a single sentence only. Avoid semicolons or colons; do not join two independent clauses.',
      '- If the target includes (sb)/(sth), replace placeholders with real words but keep the rest unchanged. You may omit a trailing placeholder only when the resulting phrase remains grammatical.',
...
export const toTranslateUkrainianSentence = async (words: WordData[]) => {
  const { reasoning, output, usage } = await generateText({
    ...
    prompt: [
      ...
      '- English translation: max 15 words, grammatical, capitalized, includes the exact target phrase. If a minimal variant is needed, limit to inflection/conjugation of verbs or auxiliaries within the target (e.g., "be going to" -> "is going to"); do not swap articles or other function words.',
