...
export const toFillInTheGap = async (words: WordData[]) => {
  const { reasoning, output, usage } = await generateText({
    ...
    prompt: [
      ...
      '- Answer: start from input word/phrase and remove (sb)/(sth). Allow only necessary inflection/conjugation; do not swap or drop other words.',
...
export const toTranslateEnglishSentence = async (words: WordData[]) => {
  const { reasoning, output, usage } = await generateText({
    ...
    prompt: [
      ...
      '- Write one English draft sentence that contains targetUsed or an allowed minimal inflected/conjugated variant exactly once as a contiguous span.',
      ...
      '<requirements>',
      '- Id: task.id matches input word.id.',
      '- English sentence: max 15 words, sentence case, natural, single sentence, and contains targetUsed or a minimal inflected/conjugated variant exactly once (case-insensitive).',
...
export const toTranslateUkrainianSentence = async (words: WordData[]) => {
  const { reasoning, output, usage } = await generateText({
    ...
    prompt: [
      ...
      '<workflow>',
      '- Build targetUsed from word: remove only (sb)/(sth). Keep all other words in the same order.',
      '- Write one English draft translation that contains targetUsed or an allowed minimal inflected/conjugated variant exactly once as a contiguous span.',
      ...
      '- English translation: max 15 words, sentence case, natural, single sentence, and contains targetUsed or a minimal inflected/conjugated variant exactly once (case-insensitive).',
      '- Target integrity: do not paraphrase or shorten the target phrase. Keep token order unchanged and do not drop, reorder, or replace function words (articles, prepositions, conjunctions, particles, modals).',
      '- Minimal variant is allowed only for verb/auxiliary inflection needed for agreement within the target phrase; function words must remain exact.',
      '- If target includes (sb)/(sth), replace placeholders with real words. A trailing placeholder may be omitted only when grammatical.',
