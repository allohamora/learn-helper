...
export const generateVocabularyItemData = async ({ value, context }: GenerateVocabularyItemDto) => {
  const { output, usage } = await generateText({
    ...
    prompt: [
      ...
      '2. Use context, if any, to pick the value\'s sense/domain/register; fall back to its most common sense if context does not help. For a fixed phrasal/prepositional verb given bare, with no argument anywhere in the input, and that has both a standalone/intransitive sense and a separate sense that takes an object (e.g. "hold on" alone, meaning to wait, vs. "hold on to (sth)", meaning to grip something), prefer the standalone sense - never pick the object-taking sense and then invent a placeholder for the object it would need.',
      '3. Write the definition, translation, spelling, and part of speech for that sense.',
      '4. Before finalizing, re-read the definition end to end and confirm every word in it is English - if a word or script from another language slipped in (e.g. echoing the value\'s own language for an emphasis word like "quickly"/"very"), rewrite that word in English.',
      '</workflow>',
      '<requirements>',
      'General:',
      ...
      '- For a fixed phrasal or prepositional verb (a verb plus particle/preposition acting as one lexical unit, e.g. "agree with", "ask for", "rely on") whose pattern takes a person/thing argument, replace that argument with "(sb)"/"(sth)"/"(sb/sth)" when the input supplies one, whether a pronoun or a noun (e.g. "agree with him" -> "agree with (sb)"; "ask for help" -> "ask for (sth)") - but never invent one for a bare input with no argument at all, even for a verb commonly used transitively elsewhere (e.g. "sign up" alone stays "sign up", not "sign up for (sth)"; only "sign up for the newsletter" becomes "sign up for (sth)"). Strip any other word not part of the fixed expression itself (e.g. a time/place adverbial like "yesterday", "outside") - the value is only the fixed expression plus its placeholder, never the full input phrase (e.g. "wait for him outside" -> "wait for (sb)"). Skip this for a plain preposition/prepositional phrase with no verb (e.g. "next to", "because of", "in front of"), for an idiom with a fixed, non-substitutable complement, and for a full clause/sentence (see isLearnable below).',
      '- For a number or ordinal number, use its spelled-out word form (e.g. "forty-two", "third"), not digits.',
      'definition:',
      '- Concise English dictionary-style definition, max 512 characters, meaning only (no examples or translations). Written entirely in English, every single word - never a stray word or script from any other language mixed in, even just one word.',
      ...
      '- Exactly one translation, always - even when genuinely torn between two valid near-synonyms for the chosen sense, commit to the single one a general bilingual English-Ukrainian dictionary would list first for it, never both. Never join two options with a semicolon, comma, "or", slash, or any other separator - the only exception is genuine grammatical variants of that one sense (e.g. gender-dependent forms), where up to two forms separated by " / " are allowed.',
