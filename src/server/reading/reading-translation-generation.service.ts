...
export const generateTranslationData = async ({ text, before, after }: TranslateSelectionDto) => {
  const { output, usage } = await generateText({
    ...
    prompt: [
      ...
      '</requirements>',
      '<examples>',
      'Illustrative only - unrelated to the actual <selection> below, never reuse their content:',
      '- selection "spill the beans", no context -> {"uaTranslation": "проговоритися", "isLearnable": true} (idiom: commit to the one standard equivalent, not a word-for-word or hedged paraphrase)',
      '- selection "trunk", context_before "The elephant raised its", context_after "to reach the leaves." -> {"uaTranslation": "хобот", "isLearnable": true} (single word: stays learnable even though context was needed to pick its sense)',
      '- selection "rush hour", no context -> {"uaTranslation": "години пік", "isLearnable": true} (fixed collocation: learnable as a unit even though "rush" and "hour" also have unrelated separate meanings)',
      '- selection "He forgot his umbrella at home again.", no context -> {"uaTranslation": "Він знову забув парасольку вдома.", "isLearnable": false} (full sentence, freshly composed: not learnable)',
      '</examples>',
